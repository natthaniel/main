package com.Main;

public class SearcherByDate implements Commander {

	/*
	 * This class will take in a date string in the form of "MM dd yyyy"
	 * and it will search from the TaskList for Tasks that has start-time on that date.
	 * Then it will get an arrayList of Task objects fulfilling the requirement.
	 * Before return, it will call the Display Class with the arrayList with the parameter.
	 * The Display Object will return a string to the processor for display.
	 * 
	 */
	public SearcherByDate() {
		// TODO Auto-generated constructor stub
	}
	
	@Override
	public String execute() {
		// TODO Auto-generated method stub
		return null;
	}

}
